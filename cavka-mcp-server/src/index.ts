...
// ── Config ──────────────────────────────────────────────
const BASE_URL =
  process.env.CAVKA_BASE_URL ?? "https://quotation-viewer.vercel.app";
const MCP_API_KEY = process.env.MCP_API_KEY ?? "";
const PORTFOLIO_EMAIL =
  process.env.CAVKA_USER_EMAIL ?? "make.some.noise6984@gmail.com";

// ── Helpers ─────────────────────────────────────────────
async function cavkaFetch<T>(path: string): Promise<T> {
...
async function cavkaAuthFetch<T>(path: string): Promise<T> {
  const url = `${BASE_URL}${path}`;
  const res = await fetch(url, {
    headers: {
...
const server = new McpServer({
  ...
  version: "1.0.0",
  description:
    "Cavka — Neo Tokyo Trading Terminal: 日本株・米国株のリアルタイム株価、ファンダメンタル、テクニカルパターン、スクリーニング機能を提供します。",
});
...
server.tool(
  "get_stock_prices",
  "複数銘柄の現在価格・前日比・変動率を一括取得。日本株はティッカー(例:7203)、米国株はシンボル(例:AAPL)で指定。USD/JPY為替レートも同時に返す。",
  {
    tickers: z
      .string()
      .describe(
        "カンマ区切りのティッカーリスト。例: '7203,4502,AAPL,MSFT'"
      ),
  },
  async ({ tickers }) => {
    const data = await cavkaFetch<{
      prices: Record<string, PriceData>;
      usdJpy: number | null;
    }>(`/api/stocks/prices?tickers=${encodeURIComponent(tickers)}`);

    const lines: string[] = ["【株価情報】"];

    for (const [ticker, p] of Object.entries(data.prices)) {
      const arrow = p.change >= 0 ? "▲" : "▼";
      lines.push(
        `${ticker}: ${fmt(p.price)} ${p.currency}  ${arrow}${fmt(Math.abs(p.change))} (${fmtPct(p.changePct)})  前日終値: ${fmt(p.prevClose)}`
      );
    }
...
server.tool(
  "get_stock_detail",
  "個別銘柄の詳細情報を取得。現在値、出来高、テクニカル指標（RSI, MACD, SMA, ボリンジャーバンド）を含む。",
  {
    ...
    range: z
      ...
      .optional()
      .describe(
        "チャート期間。'1mo','3mo','6mo','1y','2y','5y' (デフォルト: 6mo)"
      ),
  },
  async ({ ticker, range }) => {
    const r = range ?? "6mo";
    const d = await cavkaFetch<StockDetail>(
      `/api/stock/${encodeURIComponent(ticker)}?range=${r}&interval=1d`
    );
    ...
    if (ind.rsi !== null) {
      if (ind.rsi >= 70) signals.push("RSI過熱（買われすぎ）");
      else if (ind.rsi <= 30) signals.push("RSI冷却（売られすぎ）");
    }
    if (ind.macd !== null && ind.signal !== null) {
      if (ind.macd > ind.signal) signals.push("MACDゴールデンクロス圏");
      else signals.push("MACDデッドクロス圏");
    }
    ...
    if (ind.bbUpper !== null && ind.bbLower !== null) {
      if (d.price >= ind.bbUpper) signals.push("ボリンジャー上限タッチ");
      else if (d.price <= ind.bbLower) signals.push("ボリンジャー下限タッチ");
    }
...
server.tool(
  "get_fundamentals",
  "複数銘柄のファンダメンタル指標を取得。PER, PBR, PEG, EV/EBITDA, ROE, ROA, 営業利益率, 純利益率, D/Eレシオ, 流動比率, ネットキャッシュ, 売上/利益成長率, EPS, FCF, 配当利回り, 配当性向, セクター情報を含む。最大20銘柄。",
  {
    tickers: z
      .string()
      .describe(
        "カンマ区切りのティッカーリスト（最大20）。例: '7203,4502,6758'"
      ),
  },
  async ({ tickers }) => {
    const data = await cavkaFetch<{
      fundamentals: Record<string, FundamentalData>;
    }>(`/api/stocks/fundamentals?tickers=${encodeURIComponent(tickers)}`);
    ...
    for (const [, f] of Object.entries(data.fundamentals)) {
      lines.push(
        `\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`,
        `【${f.name} (${f.ticker})】 ${f.sector} / ${f.industry}`,
        `市場: ${f.market}  株価: ${fmt(f.price)}  時価総額: ${fmtLarge(f.marketCap)}`,
        ``,
        `① バリュエーション:`,
        `  PER(実績): ${fmt(f.trailingPE, 1)}  PER(予想): ${fmt(f.forwardPE, 1)}`,
        `  PBR: ${fmt(f.pbr, 2)}  PEG: ${fmt(f.pegRatio, 2)}  EV/EBITDA: ${fmt(f.evToEbitda, 1)}`,
        ``,
        `② 収益性:`,
        `  ROE: ${fmt(f.roe, 1)}%  ROA: ${fmt(f.roa, 1)}%`,
        `  営業利益率: ${fmt(f.operatingMargin, 1)}%  純利益率: ${fmt(f.profitMargin, 1)}%`,
        ``,
        `③ 財務健全性:`,
        `  D/Eレシオ: ${fmt(f.debtToEquity, 1)}  流動比率: ${fmt(f.currentRatio, 2)}`,
        `  総現金: ${fmtLarge(f.totalCash)}  総負債: ${fmtLarge(f.totalDebt)}  ネットキャッシュ: ${fmtLarge(f.netCash)}`,
        ``,
        `④ 成長性:`,
        `  売上成長率: ${fmt(f.revenueGrowth, 1)}%  利益成長率: ${fmt(f.earningsGrowth, 1)}%`,
        `  EPS(実績): ${fmt(f.epsTrailing)}  EPS(予想): ${fmt(f.epsForward)}`,
        `  FCF: ${fmtLarge(f.freeCashFlow)}`,
        ``,
        `⑤ 株主還元:`,
        `  配当利回り: ${fmt(f.dividendYield, 2)}%  配当性向: ${fmt(f.payoutRatio, 1)}%`,
        `  1株配当: ${fmt(f.dividendRate)}`
      );
    }
...
server.tool(
  "search_stocks",
  "銘柄名・ティッカー・キーワードで株式・ETF・投資信託を検索。日本語の会社名（例: 'トヨタ'）でも検索可能。",
  {
    query: z.string().describe("検索キーワード。例: 'トヨタ', 'Apple', '半導体'"),
  },
  async ({ query }) => {
    const results = await cavkaFetch<SearchResult[]>(
      `/api/stocks/search?q=${encodeURIComponent(query)}`
    );

    if (results.length === 0) {
      return {
        content: [{ type: "text" as const, text: `「${query}」に該当する銘柄が見つかりませんでした。` }],
      };
    }
    ...
    for (const r of results) {
      lines.push(
        `${r.ticker}  ${r.name}  [${r.market}] ${typeLabel[r.type] ?? r.type}  (${r.exchange})`
      );
    }
...
server.tool(
  "detect_patterns",
  "複数銘柄のチャートパターンを検出。日足・週足・月足それぞれで移動平均線の配列、トレンド方向、パターン名（ゴールデンクロス、デッドクロス、三角持ち合い等）を判定。最大20銘柄。",
  {
    tickers: z
      .string()
  ...
  async ({ tickers }) => {
    ...
    }>(`/api/stocks/patterns?tickers=${encodeURIComponent(tickers)}`);
    ...
    for (const [ticker, result] of Object.entries(data.patterns)) {
      ...
      for (const tf of ["daily", "weekly", "monthly"] as const) {
        ...
        if (pats.length === 0) {
          ...
          for (const p of pats) {
            ...
            lines.push(
              `  ${tfLabel}: ${signalEmoji} ${p.label} (${p.labelEn})  信頼度: ${(p.confidence * 100).toFixed(0)}%  強度: ${p.strength}/5`
            );
      ...
      if (result.transitions && result.transitions.length > 0) {
        ...
        for (const t of result.transitions) {
          lines.push(
            `  → ${t.label}  近接度: ${(t.proximity * 100).toFixed(0)}%`
          );
        }
      }
    }
...
server.tool(
  "screen_stocks",
  `ファンダメンタル条件で銘柄をスクリーニング。指定したティッカーリストに対してフィルタを適用し、条件を満たす銘柄を返す。
プリセット名を指定すると定型条件で検索可能:
  ...
  - "ネットキャッシュ割安": PBR≤1, ネットキャッシュ>0, PER≤15`,
  {
    tickers: z
      .string()
      .describe(
        "スクリーニング対象のティッカーリスト（カンマ区切り、最大20）。例: '7203,4502,6758,8306,9984'"
      ),
    preset: z
      .string()
      .optional()
      .describe(
        "プリセット名。'バフェット流バリュー','高配当・財務健全','成長株(GARP)','割安＋高収益','ネットキャッシュ割安' のいずれか。個別条件と併用可。"
      ),
  ...
  async (params) => {
    ...
    for (const k of filterKeys) {
      if (params[k] != null) {
        filters[k] = params[k] as number | boolean;
      }
    }
    ...
    const data = await cavkaFetch<{
      fundamentals: Record<string, FundamentalData>;
    }>(`/api/stocks/fundamentals?tickers=${encodeURIComponent(params.tickers)}`);

    // Apply filters
    const passing: FundamentalData[] = [];

    for (const f of Object.values(data.fundamentals)) {
      let pass = true;

      if (filters.perMax != null && (f.trailingPE == null || f.trailingPE > (filters.perMax as number))) pass = false;
      if (filters.pbrMax != null && (f.pbr == null || f.pbr > (filters.pbrMax as number))) pass = false;
      if (filters.pegMax != null && (f.pegRatio == null || f.pegRatio > (filters.pegMax as number))) pass = false;
      if (filters.evEbitdaMax != null && (f.evToEbitda == null || f.evToEbitda > (filters.evEbitdaMax as number))) pass = false;
      if (filters.roeMin != null && (f.roe == null || f.roe < (filters.roeMin as number))) pass = false;
      if (filters.operatingMarginMin != null && (f.operatingMargin == null || f.operatingMargin < (filters.operatingMarginMin as number))) pass = false;
      if (filters.debtToEquityMax != null && (f.debtToEquity == null || f.debtToEquity > (filters.debtToEquityMax as number))) pass = false;
      if (filters.currentRatioMin != null && (f.currentRatio == null || f.currentRatio < (filters.currentRatioMin as number))) pass = false;
      if (filters.netCashOnly === true && (f.netCash == null || f.netCash <= 0)) pass = false;
      if (filters.revenueGrowthMin != null && (f.revenueGrowth == null || f.revenueGrowth < (filters.revenueGrowthMin as number))) pass = false;
      if (filters.earningsGrowthMin != null && (f.earningsGrowth == null || f.earningsGrowth < (filters.earningsGrowthMin as number))) pass = false;
      if (filters.dividendYieldMin != null && (f.dividendYield == null || f.dividendYield < (filters.dividendYieldMin as number))) pass = false;
      if (filters.payoutRatioMin != null && (f.payoutRatio == null || f.payoutRatio < (filters.payoutRatioMin as number))) pass = false;
      if (filters.payoutRatioMax != null && (f.payoutRatio == null || f.payoutRatio > (filters.payoutRatioMax as number))) pass = false;
    ...
    if (params.preset) activeFilters.push(`プリセット: ${params.preset}`);
    if (filters.perMax != null) activeFilters.push(`PER≤${filters.perMax}`);
    if (filters.pbrMax != null) activeFilters.push(`PBR≤${filters.pbrMax}`);
    if (filters.pegMax != null) activeFilters.push(`PEG≤${filters.pegMax}`);
    if (filters.evEbitdaMax != null) activeFilters.push(`EV/EBITDA≤${filters.evEbitdaMax}`);
    if (filters.roeMin != null) activeFilters.push(`ROE≥${filters.roeMin}%`);
    if (filters.operatingMarginMin != null) activeFilters.push(`営業利益率≥${filters.operatingMarginMin}%`);
    if (filters.debtToEquityMax != null) activeFilters.push(`D/E≤${filters.debtToEquityMax}`);
    if (filters.currentRatioMin != null) activeFilters.push(`流動比率≥${filters.currentRatioMin}`);
    if (filters.netCashOnly) activeFilters.push(`ネットキャッシュのみ`);
    if (filters.revenueGrowthMin != null) activeFilters.push(`売上成長≥${filters.revenueGrowthMin}%`);
    if (filters.earningsGrowthMin != null) activeFilters.push(`利益成長≥${filters.earningsGrowthMin}%`);
    if (filters.dividendYieldMin != null) activeFilters.push(`配当利回り≥${filters.dividendYieldMin}%`);
    if (filters.payoutRatioMin != null) activeFilters.push(`配当性向≥${filters.payoutRatioMin}%`);
    if (filters.payoutRatioMax != null) activeFilters.push(`配当性向≤${filters.payoutRatioMax}%`);

    lines.push(`【スクリーニング結果】`);
    lines.push(`条件: ${activeFilters.join(", ")}`);
    lines.push(
      `対象: ${Object.keys(data.fundamentals).length}銘柄 → 該当: ${passing.length}銘柄`
    );
    ...
    if (passing.length === 0) {
      ...
      for (const f of passing) {
        ...
        lines.push(
          `  株価: ${fmt(f.price)}  時価総額: ${fmtLarge(f.marketCap)}`
        );
        lines.push(
          `  PER: ${fmt(f.trailingPE, 1)}  PBR: ${fmt(f.pbr, 2)}  PEG: ${fmt(f.pegRatio, 2)}  EV/EBITDA: ${fmt(f.evToEbitda, 1)}`
        );
        lines.push(
          `  ROE: ${fmt(f.roe, 1)}%  営業利益率: ${fmt(f.operatingMargin, 1)}%  D/E: ${fmt(f.debtToEquity, 1)}`
        );
        lines.push(
          `  売上成長: ${fmt(f.revenueGrowth, 1)}%  利益成長: ${fmt(f.earningsGrowth, 1)}%  FCF: ${fmtLarge(f.freeCashFlow)}`
        );
        lines.push(
          `  配当利回り: ${fmt(f.dividendYield, 2)}%  配当性向: ${fmt(f.payoutRatio, 1)}%`
        );
        lines.push(
          `  ネットキャッシュ: ${fmtLarge(f.netCash)}`
        );
      }
    }
...
server.tool(
  "list_nikkei225",
  "日経225構成銘柄の一覧を表示。セクター名で絞り込みも可能。screen_stocks と組み合わせてスクリーニング対象を指定する際に便利。",
  {
    filter: z
      .string()
      .optional()
      .describe(
        "銘柄名やティッカーで絞り込み。例: '自動車', '銀行', '7203'"
      ),
  },
  async ({ filter }) => {
    ...
    if (filter) {
      ...
      list = list.filter(
        (s) =>
          s.t.toLowerCase().includes(lower) ||
    ...
    lines.push(
      ``,
      `※ screen_stocks の tickers パラメータにカンマ区切りで渡すことでスクリーニングできます。`,
      `※ 一度に最大20銘柄のファンダメンタルを取得可能です。`
    );
...
interface PortfolioResponse {
  ...
  balance?: { cashJpy: number; cashUsd: number };
  transactions?: TransactionData[];
  otherAssets?: OtherAssetData[];
  watchlist?: Array<{ id: string; ticker: string; name: string; market: string; groupId: string }>;
  watchlistGroups?: Array<{ id: string; name: string; order: number }>;
  alerts?: Array<{ id: string; ticker: string; name: string; condition: string; targetPrice: number; active: boolean; triggered: boolean }>;
}

server.tool(
  "get_portfolio",
  `保有銘柄・口座残高・投資信託・外貨預金などのポートフォリオ情報を取得。現在の株価と合わせて評価損益を計算する。
取得可能なセクション: holdings(保有銘柄), balance(口座残高), transactions(取引履歴), otherAssets(投資信託・外貨), watchlist(お気に入り), alerts(アラート)`,
  {
    section: z
      ...
      .optional()
      .describe(
        "取得セクション: 'all','holdings','balance','transactions','otherAssets','watchlist','alerts' (デフォルト: all)"
      ),
    withPrices: z
      .boolean()
      .optional()
  ...
  async ({ section, withPrices }) => {
    ...
    const data = await cavkaAuthFetch<PortfolioResponse>(
      `/api/portfolio?email=${encodeURIComponent(PORTFOLIO_EMAIL)}&section=${sec}`
    );
    ...
    if (data.holdings && data.holdings.length > 0) {
      ...
      if (shouldFetchPrices) {
        ...
        try {
          const priceData = await cavkaFetch<{
            prices: Record<string, PriceData>;
            usdJpy: number | null;
      ...
      for (const h of data.holdings) {
        ...
        lines.push(``);
        lines.push(
          `  ${h.ticker} ${h.name}  [${h.market}]  ${h.shares}株`
        );
        lines.push(
          `    取得単価: ${fmt(h.avgCost)} ${ccy}  現在値: ${fmt(currentPrice)} ${ccy}`
        );
        lines.push(
          `    評価額: ${fmtLarge(value)}  取得額: ${fmtLarge(cost)}`
        );
        if (currentPrice > 0) {
          lines.push(
            `    損益: ${arrow}${fmtLarge(Math.abs(pnl))} (${fmtPct(pnlPct)})`
          );
        }
      ...
      if (shouldFetchPrices && totalCostJpy > 0) {
        ...
        if (usdJpy) lines.push(`  (USD/JPY: ${fmt(usdJpy)})`);
      }
    }
    ...
    if (data.transactions && data.transactions.length > 0) {
      ...
      for (const tx of data.transactions.slice(0, 20)) {
        ...
        lines.push(
          `  ${tx.date}  [${label}] ${ticker} ${detail}  ${tx.currency} ${fmtLarge(tx.amount)}${pnl}`
        );
      }
    }
    ...
    if (data.alerts && data.alerts.length > 0) {
      ...
      for (const a of data.alerts) {
        ...
        lines.push(
          `  ${status} ${a.ticker} ${a.name}  ${a.targetPrice}${cond}`
        );
      }
    }
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// Tool 9: get_tachibana_holdings (real broker data)
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
interface TachibanaHoldingData {
  ticker: string;
  shares: number;
...
interface TachibanaPortfolioResponse {
  ok: boolean;
  env?: string;
  lastLoginDate?: string;
  totalEvalAmount?: number;
  totalPnl?: number;
  holdings?: TachibanaHoldingData[];
  error?: string;
}

server.tool(
  "get_tachibana_holdings",
  "立花証券 e支店APIから実際の保有銘柄情報を取得する。Cavkaに手動登録した保有銘柄とは別に、証券会社が保有する正確なデータ（株数、取得単価、評価額、損益）を確認できる。現在はデモ環境に接続している。",
  {},
  async () => {
    if (!MCP_API_KEY) {
      return {
    ...
    const data = await cavkaAuthFetch<TachibanaPortfolioResponse>(
      "/api/tachibana/portfolio"
    );
    ...
    for (const h of data.holdings ?? []) {
      ...
      lines.push(
        `  取得単価: ¥${fmt(h.avgCost)}  評価単価: ¥${fmt(h.evalPrice)}`
      );
      lines.push(
        `  評価額: ¥${h.evalAmount.toLocaleString()}  損益: ${arrow}¥${Math.abs(h.pnl).toLocaleString()} (${fmtPct(h.pnlRate)})`
      );
      if (h.prevDiff !== null) {
        const da = h.prevDiff >= 0 ? "▲" : "▼";
        lines.push(
          `  前日比: ${da}¥${fmt(Math.abs(h.prevDiff))} (${fmtPct(h.prevDiffPct ?? 0)})`
        );
      }
    }
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// Tool 10: analyze_stock — 銘柄の総合分析データを取得
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
interface FullAnalysisData {
  ...
  patternTransitions: Array<Record<string, unknown>> | null;
  userContext: {
    ownsThisStock: Record<string, unknown> | null;
    inWatchlist: boolean;
    existingScenarios: Array<Record<string, unknown>>;
    totalHoldingsCount: number;
    sectorConcentration: { thisSector: string; totalHoldings: number } | null;
  };
}

server.tool(
  "analyze_stock",
  `指定銘柄の「買うべきか・買わないべきか」の意思決定に必要なデータを総合的に取得する。
取得内容:
...
このツールを呼んだ後、Claudeがすべてを統合して「買う理由3つ」「買わない理由3つ」「推奨アクション」を提示すること。`,
  ...
  async ({ ticker }) => {
    const data = await cavkaAuthFetch<FullAnalysisData>(
      `/api/stocks/full-analysis?ticker=${encodeURIComponent(ticker)}`
    );
    ...
    if (data.price) {
      ...
      lines.push(`■ 価格`);
      lines.push(
        `  現在値: ${fmt(p.current)} ${p.currency} ${arrow}${fmt(Math.abs(p.change))} (${fmtPct(p.changePct)})`
      );
      lines.push(
        `  本日: 高 ${fmt(p.dayHigh)} / 安 ${fmt(p.dayLow)} / 出来高 ${p.volume?.toLocaleString() ?? "N/A"}`
      );
    }
    ...
    if (data.priceContext) {
      ...
      lines.push(
        `  現在位置: ${pc.positionInRange}/100 (0=底, 100=天井)`
      );
      lines.push(
        `  高値から ${fmtPct(pc.fromHigh)} / 安値から ${fmtPct(pc.fromLow)}`
      );
    }
    ...
    if (data.fundamentals) {
      const f = data.fundamentals as Record<string, number | string | null>;
      lines.push(``);
      lines.push(`■ ファンダメンタル`);
      lines.push(`  業種: ${f.sector ?? "?"} / ${f.industry ?? "?"}`);
      lines.push(`  時価総額: ${fmtLarge(Number(f.marketCap) || 0)}`);
      lines.push(
        `  PER: ${fmt(Number(f.trailingPE), 1)} / PBR: ${fmt(Number(f.pbr), 2)} / PEG: ${fmt(Number(f.pegRatio), 2)}`
      );
      lines.push(
        `  ROE: ${fmt(Number(f.roe), 1)}% / 営業利益率: ${fmt(Number(f.operatingMargin), 1)}%`
      );
      lines.push(
        `  D/E: ${fmt(Number(f.debtToEquity), 1)} / 流動比率: ${fmt(Number(f.currentRatio), 2)}`
      );
      lines.push(
        `  売上成長率: ${fmt(Number(f.revenueGrowth), 1)}% / 利益成長率: ${fmt(Number(f.earningsGrowth), 1)}%`
      );
      lines.push(
        `  配当利回り: ${fmt(Number(f.dividendYield), 2)}% / 配当性向: ${fmt(Number(f.payoutRatio), 1)}%`
      );
    ...
    if (data.patterns) {
      ...
      for (const tf of ["daily", "weekly", "monthly"] as const) {
        ...
        const top = pats[0] as Record<string, unknown>;
        const signal = String(top.signal);
        const emoji = signal === "buy" ? "🟢" : signal === "sell" ? "🔴" : "🟡";
        lines.push(
          `  ${tfLabel}: ${emoji} ${top.label} (信頼度${top.confidence}%, 強度${top.strength}/5)`
        );
      ...
      if (data.patternTransitions && data.patternTransitions.length > 0) {
        ...
        for (const t of data.patternTransitions.slice(0, 3)) {
          const proxRaw = (t as Record<string, unknown>).proximity;
    ...
    if (uc.ownsThisStock) {
      const h = uc.ownsThisStock as Record<string, unknown>;
      lines.push(
        `  ✅ 既に保有中: ${h.shares}株 @ 取得単価 ¥${h.avgCost}`
      );
    } else {
      lines.push(`  ◯ 未保有`);
    }
    ...
    if (uc.existingScenarios.length > 0) {
      ...
      for (const s of uc.existingScenarios) {
        ...
        lines.push(
          `     - ${sc.direction} ¥${sc.entryPrice} → 目標¥${(sc.targetPrices as number[])[0]} / 損切り¥${sc.stopLossPrice} (${sc.status})`
        );
      }
    } else {
    ...
    if (uc.sectorConcentration) {
      lines.push(
        `  セクター: ${uc.sectorConcentration.thisSector} / 全保有銘柄: ${uc.sectorConcentration.totalHoldings}件`
      );
    }
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// Tool 11: get_my_scenarios — ユーザーのシナリオ一覧
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
server.tool(
  "get_my_scenarios",
  `ユーザーの投資シナリオを一覧取得する。各シナリオには「買う前に何を期待していたか / どんな根拠で / どんなリスクを想定していたか」が記録されている。
ユーザーの投資判断の質、バイアス傾向、過去のシナリオ vs 実際の比較分析、改善点の提案などに使用できる。`,
  {
    status: z
      ...
      .optional()
      .describe(
        "ステータスフィルタ: 'active'(監視中), 'achieved'(目標達成), 'stopped'(損切り), 'expired'(期間超過), 'manually_closed'(手動終了). 省略すると全て"
      ),
  },
  async ({ status }) => {
    const portfolioData = await cavkaAuthFetch<{ scenarios: ScenarioData[] }>(
      `/api/portfolio?email=${encodeURIComponent(PORTFOLIO_EMAIL)}&section=scenarios`
    );
    ...
    const lines: string[] = [`【投資シナリオ一覧】 ${scenarios.length}件`];
    ...
    for (const s of scenarios) {
      ...
      lines.push(`━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`);
      lines.push(
        `${s.ticker} ${s.name} - ${dirLabels[s.direction] ?? s.direction} [${statusLabels[s.status] ?? s.status}]`
      );
      lines.push(`  エントリー: ¥${s.entryPrice.toLocaleString()}`);
      lines.push(
        `  目標: ${s.targetPrices.map((p) => `¥${p.toLocaleString()}`).join(", ")}`
      );
      lines.push(`  損切り: ¥${s.stopLossPrice.toLocaleString()}`);
      lines.push(
        `  保有期間: ${s.holdingPeriod} / 確信度: ${"★".repeat(s.confidence)}${"☆".repeat(5 - s.confidence)}`
      );
      ...
      if (s.closePrice !== undefined && s.closePrice !== null) {
        ...
        lines.push(
          `  終値: ¥${s.closePrice.toLocaleString()} (${pnlPct >= 0 ? "+" : ""}${pnlPct.toFixed(2)}%)`
        );
...
function daysUntilFromIso(dateStr: string | null): number | null {
...
server.tool(
  ...
  `指定銘柄の決算カレンダー情報を取得。次回決算日、予想EPS、前回サプライズ、配当権利落・支払日が含まれる。
決算前後は値動きが激しくなるため、ポジション調整の判断材料に使う。`,
  {
    tickers: z.string().describe("カンマ区切りのティッカー（最大20）。例: '7203,6501,AAPL'"),
  },
  async ({ tickers }) => {
    const data = await cavkaFetch<{ earnings: Record<string, EarningsData> }>(
      `/api/stocks/earnings?tickers=${encodeURIComponent(tickers)}`
    );

    const lines: string[] = ["【決算カレンダー】"];
    ...
    for (const e of sorted) {
      ...
      let urgency = "";
      if (days !== null) {
        if (days < 0) urgency = ` [${Math.abs(days)}日前に発表済]`;
        else if (days === 0) urgency = " 🚨 [本日!]";
        else if (days <= 3) urgency = ` 🔴 [あと${days}日!]`;
        else if (days <= 7) urgency = ` 🟡 [あと${days}日]`;
        else urgency = ` (あと${days}日)`;
      }
      lines.push(``);
      ...
      if (e.lastEarningsSurprise !== null) {
        lines.push(
          `  前回サプライズ: ${e.lastEarningsSurprise >= 0 ? "+" : ""}${e.lastEarningsSurprise.toFixed(2)}% (実績 ${e.lastEarningsActual}/予想 ${e.lastEarningsEstimate})`
        );
      }
      if (e.exDividendDate) {
...
server.tool(
  "get_news",
  `立花証券APIから日本株のリアルタイムニュースを取得する。銘柄絞り込み・期間絞り込み可。
...
取得後、Claudeがニュースを要約・好材料/悪材料判定・投資への影響を分析できる。`,
  ...
  async ({ ticker, days, limit, fetchBody }) => {
    if (!MCP_API_KEY) {
      return {
        content: [{ type: "text" as const, text: "⚠️ MCP_API_KEY が設定されていません。" }],
      };
    }
    ...
    const data = await cavkaAuthFetch<{
      ok: boolean;
      total: number;
      items: NewsHeader[];
      error?: string;
    ...
    if (!data.ok) {
      return {
        content: [{ type: "text" as const, text: `❌ ニュース取得エラー: ${data.error}` }],
      };
    }
    ...
    if (ticker) lines.push(`銘柄絞り込み: ${ticker}`);
    lines.push(``);
    ...
    if (fetchBody) {
      for (const n of data.items.slice(0, 10)) {  // 本文取得は最大10件
        ...
        try {
          const body = await cavkaAuthFetch<{ ok: boolean; news?: { body: string } }>(
            `/api/news/${encodeURIComponent(n.id)}`
          );
          if (body.ok && body.news?.body) {
            lines.push(``);
            lines.push(body.news.body.slice(0, 800));
            if (body.news.body.length > 800) lines.push(`...(続く、合計${body.news.body.length}文字)`);
          }
        } catch {
          lines.push(`(本文取得失敗)`);
        }
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// Tool 14: get_order_book — 板情報取得 (立花API)
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
interface QuoteData {
  ticker: string;
...
server.tool(
  "get_order_book",
  `立花APIから板情報(10段)とリアルタイム時価を取得する。
板の厚みから「買い圧力 vs 売り圧力」、大口の指値の有無などを分析できる。最大120銘柄。`,
  {
    tickers: z.string().describe("カンマ区切りティッカー(最大120)。例: '7203,6501'"),
  },
  async ({ tickers }) => {
    const data = await cavkaAuthFetch<{ ok: boolean; quotes?: QuoteData[]; error?: string }>(
      `/api/tachibana/quotes?tickers=${encodeURIComponent(tickers)}`
    );
    if (!data.ok) {
      return { content: [{ type: "text" as const, text: `❌ ${data.error}` }] };
    }
    ...
    for (const q of data.quotes ?? []) {
      ...
      lines.push(
        `現在値: ¥${q.currentPrice?.toLocaleString()} ${arrow}${Math.abs(q.changeAbs ?? 0).toLocaleString()} (${(q.changePct ?? 0) >= 0 ? "+" : ""}${q.changePct?.toFixed(2)}%)`
      );
      lines.push(
        `始値¥${q.openPrice} / 高¥${q.highPrice} / 安¥${q.lowPrice} / VWAP¥${q.vwap?.toFixed(0)}`
      );
      ...
      for (let i = 9; i >= 0; i--) {
        const a = q.asks[i];
        lines.push(
          `  売${i + 1}: ¥${a.price?.toLocaleString() ?? "-"} × ${a.quantity?.toLocaleString() ?? "-"}株`
        );
      }
      ...
      for (let i = 0; i < 10; i++) {
        const b = q.bids[i];
        lines.push(
          `  買${i + 1}: ¥${b.price?.toLocaleString() ?? "-"} × ${b.quantity?.toLocaleString() ?? "-"}株`
        );
      }
    }
    return { content: [{ type: "text" as const, text: lines.join("\n") }] };
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// Tool 15: find_volume_anomalies — 出来高異常検知
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
server.tool(
  ...
  `指定銘柄リストから「いつもと違う動き」をしている銘柄を検出する。
...
heatLevel: 3=異常, 2=高, 1=やや高, 0=通常`,
  ...
  async ({ tickers, minVolumeRatio }) => {
    const data = await cavkaFetch<{ anomalies: VolumeAnomalyData[] }>(
      `/api/stocks/volume-anomaly?tickers=${encodeURIComponent(tickers)}`
    );
    ...
    const lines: string[] = [`【出来高異常検知】 ${filtered.length}件 (出来高比${min}倍以上)`];
    const heatEmoji = ["", "🟡", "🟠", "🔥"];
    ...
    for (const a of filtered) {
      ...
      lines.push(
        `  ¥${a.currentPrice} (${a.changePct >= 0 ? "+" : ""}${a.changePct}%)  出来高 ×${a.volumeRatio.toFixed(1)} (当日${(a.todayVolume / 10000).toFixed(0)}万 / 平均${(a.avgVolume20d / 10000).toFixed(0)}万)`
      );
      lines.push(
        `  モメンタム: ROC5 ${a.roc5 >= 0 ? "+" : ""}${a.roc5}% / ROC20 ${a.roc20 >= 0 ? "+" : ""}${a.roc20}%`
      );
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// Tool 16: get_margin_info — 信用残・証金残・逆日歩
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
interface MarginInfoData {
  margin: {
    ticker: string;
    date: string;
  ...
  hibu: { ticker: string; rate: number | null } | null;
  syoukin: {
    ticker: string;
    date: string;
    status: string | null;
...
最大120銘柄。`,
  ...
  async ({ tickers }) => {
    const data = await cavkaAuthFetch<{
      ok: boolean;
      data?: Record<string, MarginInfoData>;
      error?: string;
    }>(`/api/tachibana/margin?tickers=${encodeURIComponent(tickers)}`);
    ...
    for (const [ticker, info] of Object.entries(data.data ?? {})) {
      ...
      if (m) {
        ...
        lines.push(`■ 信用残 (合算)`);
        lines.push(
          `  買残: ${m.longTotal?.toLocaleString()}株 (前週比 ${m.longChangeTotal !== null && m.longChangeTotal >= 0 ? "+" : ""}${m.longChangeTotal?.toLocaleString()})`
        );
        lines.push(
          `  売残: ${m.shortTotal?.toLocaleString()}株 (前週比 ${m.shortChangeTotal !== null && m.shortChangeTotal >= 0 ? "+" : ""}${m.shortChangeTotal?.toLocaleString()})`
        );
        lines.push(`  信用倍率: ${m.ratioTotal?.toFixed(2)}倍`);

        // 解釈
        if (m.ratioTotal !== null) {
          if (m.ratioTotal < 1) lines.push(`  → 🔥 売残>買残、踏み上げ候補`);
          else if (m.ratioTotal < 3) lines.push(`  → ⚠ やや売り偏重`);
          else if (m.ratioTotal > 20) lines.push(`  → 💰 買い偏重、利食い圧力`);
          else lines.push(`  → ✓ バランスよし`);
        }
      }
...
server.tool(
  "get_future_score",
  "銘柄の将来性スコアを取得。5因子グレード (Value/Growth/Profitability/Momentum/EPS Revisions) + バリュートラップ警告 + アナリスト目標株価 + 適時開示イベント + Gemini ニュース感情を統合。「数値良いのに上がらない銘柄」を検出するための分析。",
  {
    ticker: z.string().describe("ティッカー。例: '7203' or 'AAPL'"),
  },
  async ({ ticker }) => {
    type FactorScore = { raw: number; grade: string; reasons: string[] };
    interface FutureScoreResp {
      ...
      overall: { raw: number; rawUncapped: number; grade: string; verdict: string; label: string; summary: string };
      flags: {
        valueTrap: boolean;
      ...
      disclosures?: {
        ...
        events: { date: string; type: string; headline: string }[];
      } | null;
      coverage: { confidence: number; hasAnalystData: boolean; hasSentimentData: boolean; hasDisclosureData: boolean };
    }
    const d = await cavkaFetch<FutureScoreResp>(
      `/api/stocks/future-score/${encodeURIComponent(ticker)}`
    );
    ...
    for (const [k, label] of Object.entries(factorLabels)) {
      const f = d.factors[k as keyof typeof d.factors];
      lines.push(`  ${label}: ${f.grade} (${f.raw}/100)`);
      for (const r of f.reasons.slice(0, 3)) lines.push(`    ▸ ${r}`);
    }
    lines.push(``);

    // フラグ
    const flagsList: string[] = [];
    if (d.flags.valueTrap) flagsList.push(`🚨 バリュートラップ: ${d.flags.valueTrapReason}`);
    if (d.flags.growthEmerging) flagsList.push(`🌱 グロース早期発見: ${d.flags.growthEmergingReason}`);
    if (d.flags.overhyped) flagsList.push(`🔥 過熱気味: ${d.flags.overhypedReason}`);
    if (d.flags.qualityGrowthRevoked) flagsList.push(`⚠ 高PEG/過熱により例外取消`);
    if (d.flags.cappedBy) flagsList.push(`⚠ 最弱因子 [${d.flags.cappedBy}] により減点`);
    ...
    if (d.disclosures && d.disclosures.eventCount > 0) {
      ...
      for (const [type, n] of Object.entries(d.disclosures.counts)) {
        if (n > 0) lines.push(`  ${labels[type] ?? type} ×${n}`);
      }
    ...
    if (d.sentiment && d.sentiment.articleCount > 0) {
      ...
      if (d.sentiment.stale) lines.push(`  ⚠ stale (36時間以上前)`);
      lines.push(``);
    }
...
server.tool(
  "get_score_rankings",
  "日経225+米国主要40銘柄の時間軸別スコアランキング (Cron で毎日更新)。短期/中期/長期/総合の各軸で並び替え、市場/最低スコアでフィルタ可能。",
  {
    timeframe: z
  ...
  async ({ timeframe = "overall", market = "all", minScore = 60, limit = 20 }) => {
    ...
    const filtered = (data.entries ?? [])
      ...
      .filter((e) => e[timeframe as keyof RankingEntry] as number >= minScore)
      .sort((a, b) => (b[timeframe as keyof RankingEntry] as number) - (a[timeframe as keyof RankingEntry] as number))
      .slice(0, limit);
...
server.tool(
  ...
  "当日の超短期シグナルを取得 (リアルタイム)。リバウンド候補 (急落+反発材料) / 反落警戒 (急騰+過熱) / ニュース反応中 / 代乗り判定。日経225+米国主要40を ~1秒でスキャン。",
  {
  ...
  async ({ minScore = 40, market = "all", limit = 10 }) => {
    ...
    const data = await cavkaFetch<{
      ...
      counts: { rebounds: number; overheats: number; newsActive: number };
      rebounds: ScanResult[];
      overheats: ScanResult[];
      newsActive: ScanResult[];
    }>(`/api/intraday/scan?minScore=${minScore}`);

    const filter = (list: ScanResult[]) =>
      list.filter((r) => market === "all" || r.market === market).slice(0, limit);

    const formatRow = (r: ScanResult, scoreField: "reboundScore" | "plungeScore" | "newsReactionScore") => {
...
server.tool(
  "analyze_intraday",
  "個別日本株の超短期詳細分析。立花から VWAP・板情報・直近ニュースを取得し、リバウンド/反落の精密判定。日本株のみ対応。認証必要。",
  {
  ...
  async ({ ticker }) => {
    interface AnalyzeResp {
      ...
      news: { hasRecentNews: boolean | null; newsHoursAgo: number | null };
      reboundScore: number;
      plungeScore: number;
      newsReactionScore: number;
      sympathy: { detected: boolean; direction: string | null; reason: string };
    ...
    const d = await cavkaAuthFetch<AnalyzeResp>(
      `/api/intraday/analyze/${encodeURIComponent(ticker)}`
    );
    ...
    if (d.sympathy.detected) lines.push(`  🌊 代乗り検出 (${d.sympathy.direction}): ${d.sympathy.reason}`);
    lines.push(``);
    lines.push(`━━ 検出シグナル ━━`);
    d.signals.forEach((s) => {
...
server.tool(
  "get_news_sentiment",
  "銘柄の Gemini ニュース感情キャッシュを取得。直近14日のニュースを Gemini が個別分類し、時間減衰加重平均×拡散係数で finalScore (-100〜+100) を算出。日次 cron で自動更新。",
  {
  ...
  async ({ ticker }) => {
    interface SentimentResp {
      ...
      label: string;
      computedAt: string;
      articles: { date: string; headline: string; sentiment: number; reason: string }[];
      disclosures?: { score: number; eventCount: number; counts: Record<string, number>; events: { date: string; type: string; headline: string }[] };
    }
    try {
      const d = await cavkaFetch<SentimentResp>(
        `/api/stocks/sentiment/${encodeURIComponent(ticker)}`
      );
      const lines: string[] = [];
...
server.tool(
  ...
  "銘柄の適時開示イベントを取得 (業績修正/配当/自社株買い/分割/M&A/不祥事/訴訟)。立花ニュースをキーワード分類で抽出、不祥事/訴訟は重大警告。",
  {
    ticker: z.string().describe("ティッカー。例: '7203'"),
  },
  async ({ ticker }) => {
    ...
    try {
      ...
      if (!disc || disc.eventCount === 0) {
        return { content: [{ type: "text" as const, text: `${ticker}: 直近30日に適時開示イベントなし。` }] };
      }
      const labels: Record<string, string> = {
        earnings_revision_up: "📈 上方修正", earnings_revision_down: "📉 下方修正",
        dividend_up: "💴 増配", dividend_down: "💸 減配",
      ...
      if (disc.hasCriticalNegative) lines.push(`🚨 重大開示警告: 不祥事/訴訟検出`);
      lines.push(``);
      lines.push(`━━ カテゴリ別 ━━`);
      ...
      lines.push(``);
      if (disc.latestUpRevision) lines.push(`📈 最新上方修正 [${disc.latestUpRevision.date}]: ${disc.latestUpRevision.headline}`);
      if (disc.latestDownRevision) lines.push(`📉 最新下方修正 [${disc.latestDownRevision.date}]: ${disc.latestDownRevision.headline}`);
      lines.push(``);
      lines.push(`━━ 全イベント ━━`);
      ...
      return {
        content: [{ type: "text" as const, text: `エラー: ${err instanceof Error ? err.message : String(err)}` }],
      };
    }
  }
);
